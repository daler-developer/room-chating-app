...
const entitiesSlice = createSlice({
  ...
  extraReducers(builder) {
    builder
      .addCase(
        roomsActions.fetchedFeedRooms.fulfilled,
        (state, { payload }) => {
      ...
      .addCase(
        roomsActions.fetchedRoomsUserCreated.fulfilled,
        (state, { payload }) => {
          if (payload.entities.rooms) {
            roomsAdapter.upsertMany(state.rooms, payload.entities.rooms)
          }
      ...
      .addCase(
        roomsActions.fetchedRoomsUserJoined.fulfilled,
        (state, { payload }) => {
          if (payload.entities.rooms) {
            roomsAdapter.upsertMany(state.rooms, payload.entities.rooms)
          }
      ...
      .addCase(
        authActions.profileUpdated.fulfilled,
        (state, { payload }) => {
          if (payload.entities.users) {
      ...
      .addCase(
        usersActions.fetchedFeedUsers.fulfilled,
        (state, { payload }) => {
          if (payload!.entities.users) {
            usersAdapter.upsertMany(state.users, payload!.entities.users)
          }
...
export const entitiesSelectors = {
  ...
  selectRooms(state: RootState): IRoom[] {
    const denormalized = denormalize(
      { rooms: state.entities.rooms.ids },
      { rooms: [roomSchema] },
      this.selectAllEntities(state)
    )
  ...
  selectUsers(state: RootState): IUser[] {
    const denormalized = denormalize(
      { users: state.entities.users.ids },
      { users: [userSchema] },
      this.selectAllEntities(state)
    )
  ...
  selectUserById(state: RootState, id: string): IUser | undefined {
    return usersEntitiesSelectors.selectById(state, id)
  },
