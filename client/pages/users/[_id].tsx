import { Avatar, Box, CircularProgress, Tab, Tabs, Typography } from "@mui/material"
...
export default function UserDetail () {
  ...
  const { user, error, isFetching } = useTypedSelector(usersSelectors.selectProfileUser)
  const { list: roomsCreated, isFetching: isFetchingRoomsCreated, error: fetchingRoomsCreatedError } = useTypedSelector(roomsSelectors.selectRoomsUserCreated)
  const { list: roomsJoined, isFetching: isFetchingRoomsJoined, error: fetchingRoomsJoinedError } = useTypedSelector(roomsSelectors.selectRoomsUserJoined)
  ...
  return <>
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', columnGap: '20px' }}>
    ...
    <Tabs sx={{ mt: '20px' }} variant="fullWidth" value={activeTab} onChange={(_, newValue) => setActiveTab(newValue)}>
      <Tab label="Created" />
      <Tab label="Joined" />
    </Tabs>

    <Box sx={{ mt: '20px', display: 'flex', flexDirection: 'column', rowGap: '10px' }}>
      {
        activeTab === 0 ? (
          // TAB: CREATED
