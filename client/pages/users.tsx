import { Box, Button, CircularProgress, Container, FormControl, InputLabel, MenuItem, Pagination, Select, Typography } from "@mui/material"
...
const Users = () => {
  ...
  const fetchUsers = ({ page, sort, search }: IUsersFilterObj) => {
    dispatch(usersActions.fetchedFeedUsers({
      page,
      sort,
      search
    }))
    ...
    <Box sx={{ mt: '30px' }}>
      {
        users.isFetching ? (
          ...
          <Box sx={{ mt: '30px',  display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '40px' }}>
            {
              users.list.map((user) => (
